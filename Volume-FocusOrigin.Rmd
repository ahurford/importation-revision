...
## Edits by A. Hurford June 10, 2025.
---
## load libraries
```{r}
library(dplyr)
...
## 3. TDF 
TDF <- read.csv("Data/TraVolData/TravelVolumeNLCHIfiltered.csv")
TDF$date <- as.Date(as.character(TDF$date), format = "%Y-%m-%d")
mon.TDF = read.csv("Data/TraVolData/TravelVolumeNLCHIfilteredMonthly.csv")
mon.TDF$date <- as.Date(as.character(mon.TDF$date), format = "%Y-%m-%d")
...
```
## plot TDF data & NL reported travel-related cases
```{r}
...
p1 <- ggplot() + 
  geom_point(data=TDF, aes(x=date, y= ALLCANotNL, color='CA'), size=1, alpha = 0.5) +
  geom_line(data=TDF, aes(x=date,y=rollmean(ALLCANotNL, 7, na.pad=TRUE), color='CA'),lwd=1)+
  geom_point(data=TDF, aes(x=date, y= AllFR, color= 'INT'), size=1, alpha = 0.5) +
  geom_line(data=TDF, aes(x=date,y=rollmean(AllFR, 7, na.pad=TRUE), color='INT'), lwd=1)+
...
p2 <- ggplot() + 
  geom_point(data=Travel.case, aes(x=date, y= Tot.Dom, color='CA'), size=1, alpha=0.5) +
  geom_line(data=Travel.case, aes(x=date,y=rollmean(Tot.Dom, 7, na.pad=TRUE), color='CA'),lwd=1)+
  geom_point(data=Travel.case, aes(x=date, y= Tot.Int, color= 'INT'), size=1, alpha=0.5)+
  geom_line(data=Travel.case, aes(x=date,y=rollmean(Tot.Int, 7, na.pad=TRUE), color='INT'),lwd=1)+
  
  labs(x="", y="\n Travel-related cases (daily)", title="Travel-related cases (NLHS)") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b %y") +
  scale_color_manual(values = colLeg) +
...
cut2 <- data.frame(Ref = c( "TDF data begins"),
...
p5 <- ggplot() +
  ...
  geom_text(mapping = aes(x = vals, y = 210000, label = Ref, hjust =.9, vjust =  1.5, angle = 90),
           data = cut1, size=rel(4))  +
  geom_text(mapping = aes(x = vals, y = 220000, label = Ref, hjust = 1, vjust = - 0.5, angle = 90),
           data = cut2, size=rel(4))  +
  theme_classic() + theme(axis.text.x = element_text(angle = 90, size=rel(1.5)), 
                          legend.position="bottom" ,
...
cuts <- data.frame(Ref = c("1", "2", "3", "4", "5", "6","7","8","9"),
                   vals = c(as.Date("2020-03-13"), as.Date("2020-05-04"),as.Date("2020-06-09"),
                            ...
                            as.Date("2021-03-01")), stringsAsFactors = FALSE)

cut3 <- data.frame(Ref = c("82% decrease"),
                   vals = c(as.Date("2020-10-01")),
                   stringsAsFactors = FALSE)
...
p6 <- ggplot() +
  ...
  geom_text(mapping = aes(x = vals, y = 200000, label = Ref, hjust = 1.2, vjust = -.5), 
            data = cuts, size=4, fontface="bold") +
  geom_text(mapping = aes(x = vals, y = 35000, label = Ref, hjust = .13, vjust =  0.1, angle = 0), data = cut3, size=rel(5), color= "black")  +
  geom_line(data=df.plt, aes(x=date, y= estimated, col="Total"), lty=1, size=1)+
  geom_line(data=df.plt, aes(x=date, y= IATA, col="IATA"), lty=1, size=1)+
  geom_line(data=df.plt, aes(x=date, y= TDF, col="TDF"), lty=1, size=1)+
  geom_line(data=df.plt, aes(x=date, y= FC, col="FC"), lty=1, size=1)+
  ...
  #scale_color_manual(values = colLeg) +
  scale_color_manual(values = c("#9FDA3AFF","#1FA187FF","#365C8DFF", "black")) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90, size=rel(1.5)), 
...
m1 <- pr_geographic %>%
  ...
  annotate(geom = "text", x = 642412.6, y = 201110.2, label = "27%", color = "black", size = 5)+
  annotate(geom = "text", x = 1349519.2, y = 782762.2,label = "15%", color = "white", size = 5)+
  annotate(geom = "text", x = -115306.8, y = 644233.4,label = "0.7%",color = "white", size = 5)+
  annotate(geom = "text", x = 2163606.1, y = -200409, label = "3.1%", color = "black", size = 5)+
  annotate(geom = "text", x = -1192152.6,y = 809288.2, label = "6.7%", color = "white", size = 5)+
  annotate(geom = "text", x = -1867594.7,y =912726.5, label = "2.5%",color = "white", size = 5)+
  annotate(geom = "text", x = -675361.6, y = 487741.4,label = "0.5%",color = "white", size = 5)+
  annotate(geom = "text", x = -888417.3, y = 1651194.8,label = "0.3%",color ="white", size = 5)+
  annotate(geom = "text", x = 2613606.1, y = 644233.4,label = "0.6%",color ="black", size = 5)+
  annotate(geom = "text", x = 2995757.5, y = 152727.5,label = "22%", color = "black", size = 5)+
  annotate(geom = "text", x = 1565757, y = -750409 , label = "US 14% & Others 8.2%", 
  ...
  scale_fill_continuous(type = "viridis", 
    ...
    guide = guide_colourbar(nbin = 100, draw.ulim = FALSE, draw.llim = FALSE,barwidth = 20,
                            title = "Travel volume (%)",title.theme = element_text(size=16),
...
m2 <- pr_geographic2 %>%
  ...
  annotate(geom = "text",x = 642412.6,y = 201110.2, label = "21%", color = "white",size = 5)+
  annotate(geom = "text",x = 1349519.2,y = 782762.2, label = "4.7%",color = "white",size = 5)+
  annotate(geom = "text",x = -115306.8,y = 644233.4, label = "1.0%", color = "white",size = 5)+
  annotate(geom = "text",x = 2163606.1, y = -200409, label = "3.1%", color = "black", size = 5)+
  annotate(geom = "text",x=-1192152.6,y = 809288.2, label = "16%", color = "white",size = 5)+
  annotate(geom = "text",x = -1867594.7,y =912726.5, label = "5.2%", color = "white",size = 5)+
  annotate(geom = "text",x = -675361.6,y = 487741.4, label = "0.8%", color = "white",size = 5)+
  annotate(geom = "text",x = -888417.3,y = 1651194.8 , label = "3.6%", color ="white",size = 5)+
  annotate(geom = "text", x = 2613606.1, y = 644233.4,label = "0.7%",color ="black", size = 5)+
  annotate(geom = "text",x = 2995757.5, y = 152727.5, label = "19%", color = "black", size = 5)+
  annotate(geom = "text", x = 1565757, y = -750409 , label = "US 9.2% & Others 15%", 
           fontface = "italic", color = "black", size = 5) +
  coord_fixed() +
  scale_fill_continuous( type = "viridis",
    limits = c(0,30), breaks = c(0, 5, 10, 15, 20, 25,30),
...
(p7)/(m1+m2)+plot_layout(heights = unit(c(5, 6), c('cm', 'null'))) + plot_annotation(tag_levels = 'A')
ggsave("Figure/Figure2.png", width = 9, , height = 8, dpi= 500) 
```
### Canada Map (pre and during the pandemic), show average daily over 9 month
```{r}
Av.daily1 <- function(ProFinal){
  CC <- ProFinal
  # get the average months March 2019 to Feb 2020 (except June July August 2019) 
...
C1 <- Av.daily1(ProFinal)
C2 <- Av.daily2(ProFinal)
...
m3 <- pr_geographic %>%
  ...
  geom_text(data =df.label, aes(x = new.long, y = new.lat, group = 1, label = Daily), 
            size = 5, color = "white")+ 
  annotate(geom = "text",x = 642412.6,y = 201110.2,label = "816", color = "black", size = 5)+
  annotate(geom = "text",x = 2995757.5,y = 152727.5,label = "648", color = "black", size = 5)+
  annotate(geom = "text", x = 2613606.1, y = 644233.1,label = "19",color ="black", size = 5)+
  annotate(geom = "text", x = 1465757, y = -750409 , label = "US 479 & Others 300", 
           fontface = "italic", color = "black", size = 5) +
  coord_fixed() +
  theme_mapcan() +
  scale_fill_continuous(type = "viridis",
    limits = c(0,900), breaks = c(50, 100, 200, 300, 400, 500,600, 700,800,900),
    ...
    guide = guide_colourbar(nbin = 13, draw.ulim = FALSE, draw.llim = FALSE, barwidth = 20,
                           title = "Total travel volume (Daily)", label.hjust = 1, 
                           label.vjust = 1,
                           title.theme = element_text(size=16),
                           label.theme = element_text(angle = 90, size=12))
    ) +
  theme_mapcan() + theme(legend.position="bottom",
                         plot.title=element_text(size=rel(1.5), face="bold"))+
  ggtitle("Before the pandemic")
...
m4 <- pr_geographic2 %>%
  ...
  geom_text(data =df.label, aes(x = new.long, y = new.lat, group = 1, label = Daily),
            size = 5, color = "white")+ 
  annotate(geom = "text", x = 2995757.5,y = 152727.5,label = "115", color = "black", size = 5)+
  annotate(geom = "text", x = 2613606.1, y = 633233.1,label = "5",color ="black", size = 5)+
  ...
  scale_fill_continuous( type = "viridis",
    ...
    guide = guide_colourbar(nbin = 10, draw.ulim = FALSE, draw.llim = FALSE, barwidth = 15,
                            ...
                            label.theme = element_text(angle = 90, size=9))
    ) +
  theme_mapcan() + theme(legend.position="", 
                         plot.title=element_text(size=rel(1.5), face="bold"))+
  ggtitle("During the pandemic")

(p6+p5)/(m3+m4)/(m1+m2) + plot_layout(heights = unit(c(0.4, 0.6,0.6), c('null', 'null','null'))) + plot_annotation(tag_levels = 'A')
ggsave("Figure/Figure1.png", width = 12, , height = 16, dpi= 500)
...
TDF <- select(TDF, -c("ALLCANotNL", "Bangladesh", "France", "Guyana", "India", "Norway", "Philippines", "Spain", "Suriname", "Thailand", "UK", "USA")) %>% rename(INT = AllFR)
Tra_daily2 <- TDF
...
write.csv(Tra_daily2, "Data/TraVolData/TotalTraVolume.csv", row.names = F)
```
